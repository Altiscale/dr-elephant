 ...
 */
public class MapReduceOfflineFilesFetcherHadoop2 extends MapReduceFetcher {
  private static final Logger logger = Logger.getLogger(MapReduceOfflineFilesFetcherHadoop2.class);
  ...
  public MapReduceOfflineFilesFetcherHadoop2(FetcherConfigurationData fetcherConfData) throws IOException {
    ...
    this._historyLocation = play.Configuration.root().getString("history.hdfs.path") + "/done";
    this._intermediateHistoryLocation = play.Configuration.root().getString("history.hdfs.path") + "/tmp";
  ...
  private DataFiles getHistoryFiles(AnalyticJob job) throws IOException {
    ...
    while (it.hasNext() && (jobConfPath == null || jobHistPath == null)) {
      ...
      if (name.contains(jobId)) {
        if (name.contains("_conf.xml")) {
          jobConfPath = jobHistoryDirPath + name;
        } else if (name.contains(".jhist")) {
    ...
    if (jobHistPath == null) {
      try {
        ...
        while (it.hasNext()) {
          String name = it.next().getPath().getName();
          if (name.contains(jobId) && name.contains(".jhist")) {
